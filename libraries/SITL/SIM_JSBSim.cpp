...
void JSBSim::recv_fdm(const struct sitl_input &input)
{
    ...
    rcin[4] = (fdm.spoilers + 1.0f)*0.5f; // Aux 1
    rcin[5] = (fdm.right_flap + 1.0f)*0.5f; // Aux 2
    rcin[6] = (fdm.left_aileron + 1.0f)*0.5f; // Aux 3
    rcin[7] = (fdm.elevator_trim_tab + 1.0f)*0.5f; // Camera pan
    rcin[8] = (fdm.left_flap + 1.0f)*0.5f; // Camera tilt
